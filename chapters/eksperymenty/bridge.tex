...
Jedyną ważną modyfikacją dla języka polskiego okazała się zmiana wykorzystywanego modelu \code{BERT} z wariantu \code{bert-large-uncased} na \code{bert-base-multilingual-uncased}. Oba dostępne są na platformie \code{Hugging Face}. Jest to bardzo podobne do zmiany zaaplikowanej w \code{RAT-SQL}. Uzasadnienie również jest podobne: zmieniono model na wielojęzyczny, by poradził sobie z językiem polskim oraz na mniejszy, by umożliwić naukę na posiadanym sprzęcie. 
